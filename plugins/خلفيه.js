...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  if (!text) throw `*مثال على الاستخدام ${usedPrefix + command} Zoro*`;
  ...
  try {
    ...
    if (!response.ok) {
      throw `حدث خطأ أثناء جلب الخلفية: ${response.status} ${response.statusText}`;
    }
    ...
    for (const imageUrl of randomImages) {
      ...
      if (!imageResponse.ok) {
        throw `حدث خطأ أثناء جلب الصورة: ${imageResponse.status} ${imageResponse.statusText}`;
      }
...
handler.command = /^(خلفيه|wallpaper)$/i
